...
private:
    void filter_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
    {
        ...
        for (size_t i = 0; i <= num_points_; ++i)
        {
            ...
            p_inner.x = min_threshold_ * cos(angle);
            p_inner.y = min_threshold_ * sin(angle);
            p_inner.z = 0.0;
            inner_points.push_back(p_inner);

            geometry_msgs::msg::Point p_outer;
            p_outer.x = threshold_ * cos(angle);
            p_outer.y = threshold_ * sin(angle);
            p_outer.z = 0.0;
        ...
        {
            // 第一の三角形
            fov_marker.points.push_back(inner_points[i]);
            fov_marker.points.push_back(outer_points[i]);
            fov_marker.points.push_back(inner_points[i + 1]);

            // 第二の三角形
            fov_marker.points.push_back(inner_points[i + 1]);
            fov_marker.points.push_back(outer_points[i]);
            fov_marker.points.push_back(outer_points[i + 1]);
        }
